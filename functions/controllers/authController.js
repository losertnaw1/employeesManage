...
// Environment variables (from .env or Firebase config)
const ACCESS_TOKEN_SECRET = process.env.ACCESS_TOKEN_SECRET ||
                           (functions.config().auth ? functions.config().auth.access_token_secret : null) ||
                           'access_token_secret_key';
const REFRESH_TOKEN_SECRET = process.env.REFRESH_TOKEN_SECRET ||
                            (functions.config().auth ? functions.config().auth.refresh_token_secret : null) ||
                            'refresh_token_secret_key';
const ACCESS_TOKEN_EXPIRY = '15m'; // 15 minutes
const REFRESH_TOKEN_EXPIRY = '7d'; // 7 days

// Email configuration (from .env or Firebase config)
const EMAIL_HOST = process.env.EMAIL_HOST ||
                  (functions.config().email ? functions.config().email.host : null) ||
                  'smtp.gmail.com';
const EMAIL_PORT = process.env.EMAIL_PORT ||
                  (functions.config().email ? functions.config().email.port : null) ||
                  587;
const EMAIL_USER = process.env.EMAIL_USER ||
                  (functions.config().email ? functions.config().email.user : null) ||
                  'your-email@gmail.com';
const EMAIL_PASS = process.env.EMAIL_PASS ||
                  (functions.config().email ? functions.config().email.pass : null) ||
                  'your-email-password';
const EMAIL_FROM = process.env.EMAIL_FROM ||
                  (functions.config().email ? functions.config().email.from : null) ||
                  'Employee Management System <your-email@gmail.com>';
const FRONTEND_URL = process.env.CLIENT_URL ||
                    (functions.config().app ? functions.config().app.frontend_url : null) ||
                    'https://employees-manage.web.app';
